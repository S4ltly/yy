...
public class UpdSAPxmhCronJob extends BaseCronJob {

    private Logger logger = LoggerFactory.getLogger(UpdSAPxmhCronJob.class);

    @Override
    public void execute() {
        //SAP接口地址
        String url = "/sap/bc/zcl_if_http_api/CO001?sap-client=300";

        String BUKRS = "";


//    if(!"".equals(EKORG)&&!"".equals(LIFNR)){
        try {
            ...
            String result = yyWsUtil.doPostSAP(jsonArray.toJSONString(),url);
            logger.info("内部订单返回信息");
            if (!"".equals(result)){
                JSONObject jsonObject1 = JSON.parseObject(result);
                String ztype = jsonObject1.getString("type");
                if ("S".equals(ztype)){
                    JSONArray jsonArray1 = jsonObject1.getJSONArray("item");
                    logger.info(doAction(jsonArray1));
                }
            }
        }catch (Exception e){
...
//            maintable.put("khxxh", dataObj.getString("c_khhhh"));
        ...
        //restful接口url
        HttpPost httpPost = new HttpPost("http://172.16.0.20:8081/api/cube/restful/interface/saveOrUpdateModeData/sapxmh");
